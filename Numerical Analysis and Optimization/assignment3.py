...
class Assignment3:
    ...
    def integrate(self, f: callable, a: float, b: float, n: int) -> np.float32:
        res = self.Simpson(f, a, b, n)
        result = np.float32(res)
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        res = 0
        points = Assignment2().intersections(f1, f2, 1, 100)
        if len(points) < 2:
            return np.float32('nan')
        f = lambda x: f1(x) - f2(x)
        for i in range(0, len(points) - 1):
            temp = self.integrate(f, points[i], points[i + 1], 100)
            res += abs(temp)

        result = np.float32(res)

        return result
    def Simpson(self, f, lower, higher, n):
        if n % 2 == 1:
            n = n-1
        else:
            n = n-2
        h = (higher - lower) / n
        result = f(lower) + f(higher)
        for i in range(1, n):
            k = lower + i * h
            if i % 2 == 0:
                result = result + 2 * f(k)
